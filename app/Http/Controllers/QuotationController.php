...
class QuotationController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = Quotation::with(['party', 'store', 'lines.item'])
            ->orderBy('date', 'desc')
            ->orderBy('id', 'desc');
    ...
    {
        $parties = Party::where('type', 'customer')->orWhereNull('type')->get();
        if ($parties->isEmpty()) {
            $parties = Party::all();
        }
    ...
    {
        $quotation->load(['party', 'store', 'lines.item', 'convertedInvoice']);
    ...
    public function convertToInvoice(Quotation $quotation)
    {
        if ($quotation->status === 'converted') {
            return back()->with('error', 'تم تحويل عرض السعر هذا مسبقاً إلى فاتورة.');
        }
        ...
        return redirect()->route('invoices.show', $invoice->id)->with('success', 'تم تحويل عرض السعر إلى فاتورة مبيعات بنجاح');
    }
